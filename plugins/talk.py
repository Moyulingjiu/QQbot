...
def poem():
    lineNumber = int(linecache.getline(r'poem.txt', 1))
...
def loveTalk():
    lineNumber = int(linecache.getline(r'lovetalk.txt', 1))
    a = random.randrange(2, lineNumber + 1)  # 1-9中生成随机数
    # 从文件lovetalk.txt中对读取第a行的数据
    theline = linecache.getline(r'lovetalk.txt', a)
    theline = theline[theline.find('.')+1:-1]
    return theline


def swear():
    lineNumber = int(linecache.getline(r'swear.txt', 1))
    a = random.randrange(2, lineNumber + 1)  # 1-9中生成随机数
    # 从文件swear.txt中对读取第a行的数据
    theline = linecache.getline(r'swear.txt', a)
    theline = theline[theline.find('.')+1:-1]
    return theline
...
def addLoveTalk(newLoveTalk):
    with open('lovetalk.txt', 'r+', encoding='utf-8') as f:
        text = f.readlines()
    index = 1
    print(text[0])
    text[0] = str(int(text[0]) + len(newLoveTalk))
    head = text[0]
    with open('lovetalk.txt', 'w', encoding='utf-8') as f:
        f.write(text[0] + '\n')
        del text[0]
        for line in text:
            lineTmp = line.split('.')
...
def addSwear(newLoveTalk):
    ...
    text[0] = str(int(text[0]) + len(newLoveTalk))
    head = text[0]
    with open('swear.txt', 'w', encoding='utf-8') as f:
        f.write(text[0] + '\n')
        del text[0]
        for line in text:
            lineTmp = line.split('.')
