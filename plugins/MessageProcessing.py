...
class MessageProcessing:
    ...
    async def run(self, app, mode, member, message_source):
        ...
        if mode == 1 and (group_right < 2 or right < 3):
            if message_code == '*quit':
                await app.sendGroupMessage(member.group, MessageChain.create([
                    Plain('再见啦~各位！我会想你们的')
                ]))
                ...
                logManage.groupLog(getNow.toString(), qq, group_id, member.group.name, message + '; 小柒解出禁言！')
                return
        ...
        if message_code[:5] == '*send':
            ...
            if mode == 0 or mode == 2:
                ...
                await send_message(app, mode, member, reply_text, reply_image, need_at, at_qq)
            elif mode == 1 and (group_right < 2 or right < 3):  # 如果是群聊则需要有权限，才能够操作
            ...
            if mode == 0 or mode == 2:
                ...
                await send_message(app, mode, member, reply_text, reply_image, need_at, at_qq)
            elif mode == 1 and (group_right < 2 or right < 3):
        ...
        # 指令
        if not need_reply:
            if message[0] == '*' and message[1] != '由' and message[1] != '*':
