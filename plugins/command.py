...
def helpContributor():
    ...
    result += '2.添加回复 关键词 回复内容：添加一个关键词回复，如果输入这个关键词，小柒将会自动回复回复内容\n'
    result += '3.删除回复 关键词 回复内容：删除一个关键词回复配对\n'
    result += '4.添加回复*关键词*回复内容：（以*分割，可以带空格）添加一个关键词回复，如果输入这个关键词，小柒将会自动回复回复内容\n'
    result += '5.删除回复*关键词*回复内容：（以*分割，可以带空格）删除一个关键词回复配对\n'
    result += '6.打卡帮助：来获取对于打卡计划的管理帮助\n'
    result += '--------------------------\n'
...
def helpMaster():
    ...
    result += '12.查看管理员：查看管理员名单\n'
    result += '13.添加管理员 账号：向管理员中加入新人\n'
    result += '14.删除管理员 账号：向管理员中移除某个人\n'
    result += '--------------------------\n'
    result += '注：小柒的操作权限分为4类，权限向下兼容，由高到低分别为：主人、管理员、贡献者、用户\n'
    return result
